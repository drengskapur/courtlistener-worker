...
pub async fn fetch_court(id: &str, env: &Env, req: &Request) -> Result<Response> {
    let court: crate::ApiCourt = ApiClient::fetch_json(env, &format!("/courts/{}/", id), req).await?;
    json_response(&court)
}
...
pub async fn fetch_opinion_clusters(env: &Env, req: &Request) -> Result<Response> {
    let clusters: crate::OpinionClustersResponse = ApiClient::fetch_json(env, "/clusters/", req).await?;
...
pub async fn fetch_search_post(env: &Env, req: &Request, body: &str) -> Result<Response> {
    ...
    let init = RequestInit {
        method: Method::Post,
        headers,
        body: if body.is_empty() { None } else { Some(JsValue::from_str(body)) },
        ..Default::default()
    };
...
pub async fn fetch_citations(env: &Env, req: &Request) -> Result<Response> {
    let citations: crate::CitationsResponse = ApiClient::fetch_json(env, "/opinions-cited/", req).await?;
...
pub async fn stream_audio_file(req: &Request, env: &Env) -> Result<Response> {
    ...
    let audio_url = if let Some(url_param) = url.query_pairs().find(|(k, _)| k == "url") {
        ...
        let audio: serde_json::Value = ApiClient::fetch_json(env, &format!("/audio/{}/", audio_id), req).await?;
    ...
    audio_req.headers_mut()?.set("User-Agent", &format!("courtlistener-worker/{}", env!("CARGO_PKG_VERSION")))?;

...
mod tests {
    ...
    async fn test_fetch_courts() {
        let json = fetch_json("/courts/").await.expect("Failed to fetch courts");
        
        let courts: CourtsResponse = serde_json::from_str(&json)
            .expect("Failed to parse courts response");
    ...
    async fn test_fetch_specific_court() {
        let json = fetch_json("/courts/scotus/").await
            .expect("Failed to fetch SCOTUS");
        
        let court: ApiCourt = serde_json::from_str(&json)
            .expect("Failed to parse court response");

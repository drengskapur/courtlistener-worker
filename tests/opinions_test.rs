...
mod tests {
    ...
    async fn test_fetch_opinions() {
        let json = fetch_json("/opinions/?page_size=5").await
        ...
        if value.get("results").is_some() {
            let opinions: OpinionsResponse = serde_json::from_str(&json)
                .expect("Failed to parse opinions response");
    ...
    async fn test_fetch_opinion_clusters() {
        let json = fetch_json("/opinion-clusters/?page_size=5").await
            .expect("Failed to fetch opinion clusters");
        
        let value: serde_json::Value = match serde_json::from_str(&json) {
            Ok(v) => v,
            Err(e) => {
                println!("Opinion clusters API returned non-JSON (might require auth): {}", e);
        ...
        if value.get("results").is_some() {
            match serde_json::from_str::<OpinionClustersResponse>(&json) {
                ...
                Err(e) => {
                    println!("Failed to parse opinion clusters response (might require auth): {}", e);
                }
            }
        }
    }

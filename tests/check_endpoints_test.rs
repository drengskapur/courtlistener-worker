...
mod tests {
    ...
    async fn test_check_endpoints_structure() {
        if !check_worker_available().await {
            eprintln!(
                "Skipping test: Worker not available at {}",
                get_worker_url()
            );
            return;
        }
        ...
        let response = reqwest::get(&url)
            .await
            .expect("Failed to fetch check-endpoints");
        let json: serde_json::Value = response.json().await.expect("Failed to parse JSON");

        // Verify top-level structure
        assert!(
            json.get("api_endpoints").is_some(),
            "Missing 'api_endpoints' field"
        );
        assert!(
            json.get("our_endpoints").is_some(),
            "Missing 'our_endpoints' field"
        );
    ...
    async fn test_check_endpoints_api_endpoints() {
        ...
        assert!(
            api_endpoints.get("total").is_some(),
            "Missing 'total' in api_endpoints"
        );
        assert!(
            api_endpoints.get("list").is_some(),
            "Missing 'list' in api_endpoints"
        );

        let total = api_endpoints
            .get("total")
            .and_then(|v| v.as_u64())
        ...
        assert!(
            endpoints.contains(&"courts".to_string()),
            "Should include 'courts' endpoint"
        );
        assert!(
            endpoints.contains(&"opinions".to_string()),
            "Should include 'opinions' endpoint"
        );
        assert!(
            endpoints.contains(&"people".to_string()),
            "Should include 'people' endpoint"
        );
    }
    ...
    async fn test_check_endpoints_our_endpoints() {
        ...
        assert!(
            our_endpoints.get("total").is_some(),
            "Missing 'total' in our_endpoints"
        );
        assert!(
            our_endpoints.get("list").is_some(),
            "Missing 'list' in our_endpoints"
        );

        let total = our_endpoints
            .get("total")
            .and_then(|v| v.as_u64())
            .expect("total should be a number");
        assert!(total > 0, "We should support at least one endpoint");

        let list = our_endpoints
            .get("list")
            .and_then(|v| v.as_array())
            .expect("list should be an array");
        assert_eq!(list.len() as u64, total, "List length should match total");
        ...
        assert!(
            endpoints.contains(&"courts".to_string()),
            "Should support 'courts'"
        );
        assert!(
            endpoints.contains(&"opinions".to_string()),
            "Should support 'opinions'"
        );
        assert!(
            endpoints.contains(&"people".to_string()),
        ...
        assert!(
            endpoints.contains(&"dockets".to_string()),
            "Should support 'dockets'"
        );
        assert!(
            endpoints.contains(&"search".to_string()),
    ...
    async fn test_check_endpoints_coverage() {
        ...
        assert!(
            coverage.get("percentage").is_some(),
            "Missing 'percentage' in coverage"
        );
        assert!(
            coverage.get("all_covered").is_some(),
            "Missing 'all_covered' in coverage"
        );

        let percentage = coverage
            .get("percentage")
            .and_then(|v| v.as_f64())
        ...
        let all_covered = coverage
            .get("all_covered")
            .and_then(|v| v.as_bool())
            .expect("all_covered should be a boolean");
    ...
    async fn test_check_endpoints_coverage_consistency() {
        ...
        if api_total > 0 {
            ...
            assert!(
                (coverage_percentage - expected_percentage).abs() < 0.01,

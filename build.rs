...
fn fetch_latest_version_from_github() -> Option<String> {
        ...
        .build()
    {
        Ok(c) => c,
        Err(e) => {
            eprintln!("cargo:warning=Failed to create HTTP client: {} (using local versions)", e);
            return None;
        }
    };
    ...
    let response = match client.get(url).send() {
        Ok(r) => r,
        Err(e) => {
            eprintln!("cargo:warning=Failed to fetch version from GitHub: {} (using local versions)", e);
            return None;
        }
    };
    ...
    let html = match response.text() {
        Ok(h) => h,
        Err(e) => {
            eprintln!("cargo:warning=Failed to parse GitHub response: {} (using local versions)", e);
            return None;
        }
    };
    ...
    let re = match Regex::new(r"<strong>v(\d+)\.(\d+)") {
        Ok(r) => r,
        Err(e) => {
            eprintln!("cargo:warning=Failed to create regex: {} (using local versions)", e);
            return None;
        }
    };
    ...
    eprintln!("cargo:warning=Could not find version pattern in GitHub changelog (using local versions)");
    None
}
...
fn fetch_openapi_spec_from_api(version: &str) -> Option<String> {
    ...
    let major_version = version.strip_prefix('v')
        .and_then(|v| v.split('.').next())
        .unwrap_or("4");
    
    let api_version_path = format!("v{}", major_version);
    let api_base = format!("https://www.courtlistener.com/api/rest/{}/", api_version_path);
        ...
        .build()
    {
        Ok(c) => c,
        Err(e) => {
            eprintln!("cargo:warning=Failed to create HTTP client: {} (skipping auto-fetch)", e);
            return None;
        }
    };
    ...
    let response = match client.get(&api_base).send() {
        Ok(r) => r,
        Err(e) => {
            eprintln!("cargo:warning=Failed to fetch API root: {} (skipping auto-fetch)", e);
            return None;
        }
    };
    ...
    if !response.status().is_success() {
        eprintln!("cargo:warning=API returned status {} (skipping auto-fetch)", response.status());
        return None;
    }
    ...
    let root: serde_json::Value = match response.json() {
        Ok(r) => r,
        Err(e) => {
            eprintln!("cargo:warning=Failed to parse API root response: {} (skipping auto-fetch)", e);
            return None;
        }
    };
                ...
                if url.starts_with("http") {
                    ...
                    if let Some(path_start) = url.find(&api_path_pattern) {
                        ...
                        if let Some(query_start) = path.find('?') {
                            ...
                            paths.insert(clean_path.clone(), serde_json::json!({
                                "get": {
...
fn main() {
    ...
    if versions.is_empty() {
        ...
        let version_to_fetch = latest_version_from_github.clone().unwrap_or_else(|| {
            println!("cargo:warning=GitHub fetch failed and no local versions, using fallback 4.4.0");
            "4.4.0".to_string()
        });
        
        println!("cargo:warning=No local versions found, attempting to fetch {} from API...", version_to_fetch);
        ...
        if let Some(spec_content) = fetch_openapi_spec_from_api(&api_version) {
            ...
            fs::create_dir_all(&version_dir).unwrap_or_else(|e| {
                panic!("Failed to create directory {}: {}", version_dir.display(), e);
            });
            let spec_path = version_dir.join("openapi.json");
            fs::write(&spec_path, spec_content).unwrap_or_else(|e| {
                panic!("Failed to write openapi.json to {}: {}", spec_path.display(), e);
            });
            println!("cargo:warning=Successfully fetched and saved OpenAPI spec for {}", version_to_fetch);
    ...
    let (target_version_dir, (major, minor, patch)) = if let Some(ref github_version) = latest_version_from_github {
        ...
        if let Some((dir, ver)) = versions.iter().find(|(dir, _)| {
            dir == github_version || dir == &version_with_v
        }) {
            // Use the GitHub version that exists locally
            (dir.clone(), *ver)
        } else {
            // GitHub version not found locally - try to fetch it
            println!("cargo:warning=GitHub version {} not found locally, attempting to fetch...", github_version);
            // Use version with "v" prefix for API calls
            ...
            if let Some(spec_content) = fetch_openapi_spec_from_api(&api_version) {
                ...
                println!("cargo:warning=Successfully fetched and saved OpenAPI spec for {}", github_version);
                ...
                println!("cargo:warning=Failed to fetch {}, using highest available: {}", 
